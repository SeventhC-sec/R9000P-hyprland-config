...
⚠️ **可兼容但需修改分辨率**：
- 联想拯救者 R9000P 2022（默认 2560x1600@165，无需修改）
- 联想拯救者 R9000P 2021（必须改为 2560x1440@165）
...
### 1. 安装必备依赖
```bash
sudo pacman -S --needed hyprland waybar rofi kitty swaync hyprlock hypridle wallust swww python jq cliphist swappy grim slurp wl-clipboard polkit-gnome
 
...
cd ~
git clone https://github.com/SeventhC-sec/R9000P-hyprland-config.git
cd R9000P-hyprland-config
chmod +x install.sh
./install.sh
 
...
Q: 如何恢复我原来的配置？
A:  install.sh  会自动备份旧配置到  ~/.config/xxx.bak ，将备份文件夹改回原名即可恢复。
